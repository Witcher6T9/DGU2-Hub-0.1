...
import {
  ...
  clearAllBackupsFromIndexedDB,
  downloadBackupRecordAsJSON,
...
export const DatabaseModal: React.FC<DatabaseModalProps> = ({
  ...
  // IndexedDB Daily Backup State
  const [indexedDbBackups, setIndexedDbBackups] = useState<DailyBackupRecord[]>([]);
  const [isLoadingBackups, setIsLoadingBackups] = useState<boolean>(false);
  const [isBackingUpNow, setIsBackingUpNow] = useState<boolean>(false);
  ...
  const loadIndexedDbData = async () => {
    ...
    try {
      ...
      setIndexedDbBackups(backups);
      setStorageInfo(storage);
    } catch (err) {
      console.error('Failed to load IndexedDB backups:', err);
    } finally {
  ...
  const handleRunImmediateBackup = async () => {
    ...
    try {
      let record: DailyBackupRecord;
      if (onTriggerManualBackup) {
        record = await onTriggerManualBackup();
      } else {
        const payload: AppBackupState = {
          lines,
          checklists,
          todos,
          leanActions,
        ...
        record = await saveBackupToIndexedDB(payload, 'manual');
      }

      setBackupNotice({
        text: `IndexedDB snapshot saved successfully! (${record.stats.linesCount} lines, ${record.stats.checklistsDaysCount} days checklists, ${record.stats.todosCount} tasks · ${record.stats.sizeFormatted})`,
        type: 'success'
      });
      await loadIndexedDbData();
    } catch (err: any) {
      setBackupNotice({
        text: `Failed to save IndexedDB backup: ${err?.message || 'Storage error'}`,
        type: 'error'
      });
    } finally {
  ...
  const handleRestoreFromSnapshot = (record: DailyBackupRecord) => {
    if (
      confirm(
        `Restore application state from IndexedDB snapshot (${record.date} ${record.timeStr})?\n\nThis will restore ${record.stats.linesCount} lines, ${record.stats.checklistsDaysCount} days of checklists, and ${record.stats.todosCount} floor tasks into your active factory dashboard.`
      )
    ) {
      onRestoreData(record.data);
      setBackupNotice({
        text: `Database successfully restored from IndexedDB snapshot (${record.date} ${record.timeStr})!`,
        type: 'success'
      });
      setTimeout(() => {
        onClose();
  ...
  const handleDeleteSnapshot = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (confirm('Delete this backup snapshot from browser IndexedDB?')) {
      try {
        await deleteBackupFromIndexedDB(id);
        await loadIndexedDbData();
        setBackupNotice({ text: 'Snapshot deleted from IndexedDB.', type: 'success' });
      } catch (err: any) {
        setBackupNotice({ text: `Failed to delete snapshot: ${err?.message}`, type: 'error' });
      }
    }
  };
  ...
  const handleClearAllIndexedDb = async () => {
    if (
      confirm(
        `Are you sure you want to delete ALL ${indexedDbBackups.length} local IndexedDB daily backup snapshots? This action cannot be undone.`
      )
    ) {
      try {
        await clearAllBackupsFromIndexedDB();
        await loadIndexedDbData();
        setBackupNotice({ text: 'All IndexedDB snapshots cleared.', type: 'success' });
      } catch (err: any) {
        setBackupNotice({ text: `Failed to clear snapshots: ${err?.message}`, type: 'error' });
      }
    }
  };
      ...
      >
        ...
        {activeTab === 'backup' && (
          <div className="p-6 space-y-5 overflow-y-auto">
            ...
            {/* AUTOMATED LOCAL DAILY BACKUP TO INDEXEDDB (NEW FEATURE) */}
            <div className="p-4 sm:p-5 rounded-2xl bg-white border-2 border-[#176f78]/30 shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2 border-b border-[#e7e1d5]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="text-xs font-bold uppercase tracking-tight text-[#17343a]">
                    ...
                    <p className="text-[11px] text-[#527078]">
                      Scheduled daily snapshot of lines, checklists &amp; todos in browser flash storage
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-[#527078] bg-[#f1eee6] px-2.5 py-1 rounded-lg font-mono-numbers">
                    {storageInfo.formattedUsage} used ({indexedDbBackups.length} snapshots)
                  </span>
                  {onOpenSettingsBackup && (
                    <button
                      type="button"
                      ...
                      className="p-1.5 rounded-lg border border-[#d9d2c2] hover:bg-[#f1eee6] text-[#176f78] text-xs font-bold transition-colors cursor-pointer flex items-center gap-1"
                      title="Configure Daily Backup Schedule in Settings"
                    >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs">
                <div className="p-3 rounded-xl bg-[#fbfaf6] border border-[#e7e1d5]">
                  <div className="text-[10px] font-bold uppercase text-[#527078] flex items-center gap-1">
                    <Clock className="w-3 h-3 text-[#176f78]" />
                    <span>Daily Schedule Time</span>
                  </div>
                  <div className="font-bold text-sm text-[#17343a] mt-1 font-mono-numbers">
                    {dailyBackupSettings?.dailyBackupTime || '18:00'} (24-Hour)
                  </div>
                  <div className="text-[10px] text-emerald-700 font-semibold mt-0.5">
                    {dailyBackupSettings?.autoDailyBackupEnabled !== false
                ...
                <div className="p-3 rounded-xl bg-[#fbfaf6] border border-[#e7e1d5]">
                  ...
                  <div className="font-bold text-sm text-[#17343a] mt-1 truncate">
                    {dailyBackupSettings?.lastBackupDate === new Date().toISOString().split('T')[0]
                      ? '✓ Completed Today'
                      : `Pending (at ${dailyBackupSettings?.dailyBackupTime || '18:00'})`}
                  </div>
                  <div className="text-[10px] text-[#527078] mt-0.5 truncate">
                    {dailyBackupSettings?.lastBackupSummary || 'Retention: 30 days'}
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-[#fbfaf6] border border-[#e7e1d5]">
                  <div className="text-[10px] font-bold uppercase text-[#527078] flex items-center gap-1">
                    ...
                    <span>Engine &amp; Privacy</span>
                  </div>
                  <div className="font-bold text-sm text-[#17343a] mt-1">
                    IndexedDB Local
                  </div>
              ...
              {/* Trigger Now Button & Action Row */}
              <div className="flex flex-col sm:flex-row items-center gap-2">
                <button
                  type="button"
                  id="run-daily-backup-now-btn"
                  onClick={handleRunImmediateBackup}
                  disabled={isBackingUpNow}
                  className="w-full sm:flex-1 py-2.5 px-4 rounded-xl bg-[#176f78] hover:bg-[#12555c] text-white text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs disabled:opacity-50"
                >
                  {isBackingUpNow ? (
                    <>
                      <RefreshCw className="w-4 h-4 animate-spin" />
                      <span>Saving Snapshot to IndexedDB...</span>
                    </>
                  ) : (
                    <>
                      <ArrowDownToLine className="w-4 h-4" />
                      <span>Run Daily Backup to IndexedDB Now</span>
                    </>
                  )}
                </button>
                ...
                <button
                  ...
                  className="w-full sm:w-auto py-2.5 px-3 rounded-xl bg-[#f1eee6] hover:bg-[#e7e1d5] text-[#17343a] text-xs font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                  title="Refresh snapshot list from IndexedDB"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isLoadingBackups ? 'animate-spin' : ''}`} />
                  <span>Refresh</span>
                </button>

                {indexedDbBackups.length > 0 && (
                  <button
                    type="button"
                    onClick={handleClearAllIndexedDb}
                    className="w-full sm:w-auto py-2.5 px-3 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold transition-colors flex items-center justify-center gap-1.5 cursor-pointer border border-rose-200"
                    title="Clear all stored IndexedDB snapshots"
                  >
              ...
              {/* Saved Snapshots Repository / History */}
              <div className="space-y-2 pt-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-[#17343a] flex items-center gap-1.5">
                    <Archive className="w-3.5 h-3.5 text-[#176f78]" />
                    <span>IndexedDB Snapshots Repository ({indexedDbBackups.length})</span>
                  </span>
                  <span className="text-[10px] text-[#527078]">
                    Click "Restore" on any snapshot to recover that exact factory state
                  </span>
                </div>

                {indexedDbBackups.length === 0 ? (
                  <div className="p-4 rounded-xl bg-[#fbfaf6] border border-dashed border-[#d9d2c2] text-center text-xs text-[#527078]">
                    <div className="w-8 h-8 rounded-full bg-[#f1eee6] flex items-center justify-center mx-auto mb-1.5 text-[#176f78]">
                      <Clock className="w-4 h-4" />
                    </div>
                    <div className="font-bold text-[#17343a]">No Daily Backups Stored Yet in IndexedDB</div>
                    <div className="text-[11px] mt-0.5">
                      Snapshots trigger automatically once per day at {dailyBackupSettings?.dailyBackupTime || '18:00'}, or you can click "Run Daily Backup to IndexedDB Now" above.
                    </div>
                  </div>
                ) : (
                  <div className="max-h-52 overflow-y-auto space-y-1.5 pr-1">
                    {indexedDbBackups.map(snapshot => (
                      <div
                        key={snapshot.id}
                        className="p-2.5 rounded-xl border border-[#e7e1d5] bg-[#fbfaf6] hover:bg-white hover:border-[#176f78]/40 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${
                              snapshot.triggerType === 'scheduled'
                                ? 'bg-emerald-500'
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-bold text-[#17343a] font-mono-numbers">
                                {snapshot.date} {snapshot.timeStr}
                              </span>
                              <span
                                className={`text-[9px] uppercase px-1.5 py-0.2 rounded-md font-bold font-mono ${
                                  snapshot.triggerType === 'scheduled'
                            ...
                            <div className="text-[10px] text-[#527078] flex items-center gap-2 mt-0.5 flex-wrap">
                              <span>{snapshot.stats.linesCount} lines</span>
                              <span>•</span>
                              <span>{snapshot.stats.checklistsDaysCount} days checklist</span>
                              <span>•</span>
                              <span>{snapshot.stats.todosCount} tasks</span>
                              <span>•</span>
                              <span className="font-mono text-emerald-700 font-semibold">
                                {snapshot.stats.sizeFormatted}
                              </span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                          <button
                            type="button"
                            onClick={() => handleRestoreFromSnapshot(snapshot)}
                            className="px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 text-[11px] font-bold transition-colors cursor-pointer flex items-center gap-1"
                            title="Restore application state from this snapshot"
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => downloadBackupRecordAsJSON(snapshot)}
                            className="p-1 rounded-lg bg-white border border-[#d9d2c2] hover:bg-[#f1eee6] text-[#527078] text-[11px] font-bold transition-colors cursor-pointer"
                          ...
                          <button
                            ...
                            className="p-1 rounded-lg bg-white border border-rose-200 hover:bg-rose-50 text-rose-600 text-[11px] font-bold transition-colors cursor-pointer"
                            title="Delete snapshot from IndexedDB"
                          >
