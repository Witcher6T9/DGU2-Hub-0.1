...
export const IESimulator: React.FC<IESimulatorProps> = ({
  ...
  const [matrixProgressionDay, setMatrixProgressionDay] = useState<number>(6);
  const [isMatrixEffMode, setIsMatrixEffMode] = useState<boolean>(true);
  const [targetEffPct, setTargetEffPct] = useState<number>(() => {
    const initWeight = getSMVWeight(activePreset.totalSMV);
    return getProgressionTargetEff(6, 'new', initWeight);
  });
  ...
  useEffect(() => {
    if (isMatrixEffMode) {
      const standardEff = getProgressionTargetEff(matrixProgressionDay, simStyleNature, smvWeight);
      setTargetEffPct(standardEff);
    }
  }, [matrixProgressionDay, simStyleNature, smvWeight, isMatrixEffMode]);
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <div className="p-4 rounded-2xl bg-white border border-[#e7e1d5] border-l-[4px] border-l-emerald-500 shadow-2xs">
              ...
              <span className="text-[10px] text-emerald-700 font-bold mt-1 block truncate" title={`Standard 40-Day Matrix: Day ${matrixProgressionDay} (${simStyleNature === 'new' ? 'New' : 'Repeat'}, ${smvWeight})`}>
                {isMatrixEffMode
                  ? `40-Day Matrix: Day ${matrixProgressionDay} (${simStyleNature === 'new' ? 'New' : 'Repeat'})`
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 rounded-2xl border border-[#d9d2c2] bg-[#fbfaf6] p-5 shadow-xs space-y-5">
              ...
              {/* Target Efficiency Level - Standard 40-Day Style Progression Matrix */}
              <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-br from-[#f8faf9] to-[#f4f7f6] border border-[#cbdad7] shadow-2xs space-y-3">
                {/* Header row */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="w-6 h-6 rounded-lg bg-[#176f78] text-white flex items-center justify-center shrink-0 shadow-2xs">
                      <TrendingUp className="w-3.5 h-3.5" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-bold text-xs uppercase tracking-wide text-[#17343a]">
                          Target Efficiency Level
                        </span>
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wide bg-emerald-100 text-emerald-800 border border-emerald-300">
                          Style Progression 40-Day Matrix
                        </span>
                      </div>
                      <span className="text-[11px] text-[#527078]">
                        Standard {simStyleNature === 'new' ? 'New Style' : 'Repeat Style'} • {smvWeight.toUpperCase()} SMV ({smvMinutes.toFixed(1)}m)
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 shrink-0 self-end sm:self-center">
                    <div className="text-right">
                      <div className="flex items-baseline gap-1 justify-end">
                        <span className="font-mono-numbers text-xl sm:text-2xl font-black text-emerald-700">
                          {targetEffPct}%
                        </span>
                        <span className="text-[10px] font-bold uppercase text-[#527078]">Target</span>
                      </div>
                      <span className="text-[10px] font-mono-numbers font-bold text-[#176f78] block">
                        {isMatrixEffMode ? `Day ${matrixProgressionDay} of 40` : 'Custom Override'}
                      </span>
                    </div>
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 rounded-xl bg-[#176f78] hover:bg-[#125860] text-white font-bold text-xs flex items-center gap-1.5 shadow-xs transition-colors cursor-pointer shrink-0"
                      title="Open full Standard 40-Day Style Progression Matrix Chart"
                    >
                      <Calendar className="w-3.5 h-3.5" />
                      <span className="hidden sm:inline">40-Day Chart</span>
                      <span className="sm:hidden">Chart</span>
                    </button>
                  </div>
                </div>

                {/* Classification & Mode Controls Strip */}
                <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-[#dce8e6] text-xs">
                  {/* Style Nature Toggle */}
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] font-bold uppercase text-[#527078]">Style Nature:</span>
                    <div className="inline-flex rounded-lg bg-white border border-[#cbdad7] p-0.5 shadow-2xs">
                      <button
                        type="button"
                        onClick={() => {
                          setSimStyleNature('new');
                          if (isMatrixEffMode) {
                            const newEff = getProgressionTargetEff(matrixProgressionDay, 'new', smvWeight);
                            setTargetEffPct(newEff);
                          }
                        }}
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setSimStyleNature('repeat');
                          if (isMatrixEffMode) {
                            const newEff = getProgressionTargetEff(matrixProgressionDay, 'repeat', smvWeight);
                            setTargetEffPct(newEff);
                          }
                        }}
                        className={`px-2 py-0.5 rounded-md text-[10px] font-bold transition-all cursor-pointer ${
                          simStyleNature === 'repeat'
                            ? 'bg-[#8c531b] text-white shadow-2xs'
                            : 'text-[#527078] hover:text-[#17343a]'
                        }`}
                      >
                        Repeat (≤ 3 mos)
                  ...
                  {/* Mode Switcher */}
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] font-bold uppercase text-[#527078]">Mode:</span>
                    <div className="inline-flex rounded-lg bg-white border border-[#cbdad7] p-0.5 shadow-2xs">
                      <button
                        type="button"
                        onClick={() => {
                          setIsMatrixEffMode(true);
                          const newEff = getProgressionTargetEff(matrixProgressionDay, simStyleNature, smvWeight);
                          setTargetEffPct(newEff);
                        }}
                        className={`px-2 py-0.5 rounded-md text-[10px] font-bold transition-all cursor-pointer ${
                          isMatrixEffMode
                      ...
                      <button
                        type="button"
                        onClick={() => setIsMatrixEffMode(false)}
                        className={`px-2 py-0.5 rounded-md text-[10px] font-bold transition-all cursor-pointer ${
                          !isMatrixEffMode
                ...
                {/* 40-Day Matrix Slider or Custom Slider */}
                {isMatrixEffMode ? (
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-bold text-[#17343a]">
                        Progression Day on Matrix (1 – 40):
                      </span>
                      <span className="font-mono-numbers font-bold text-[#176f78]">
                        Day {matrixProgressionDay} = <strong className="text-emerald-700 font-extrabold">{targetEffPct}% Target Efficiency</strong>
                      </span>
                    </div>
                    ...
                    <input
                      ...
                      max={40}
                      step={1}
                      value={matrixProgressionDay}
                      onChange={e => {
                        const d = parseInt(e.target.value);
                        setMatrixProgressionDay(d);
                        if (d <= 6) setSimLcCurrentDay(d);
                        setTargetEffPct(getProgressionTargetEff(d, simStyleNature, smvWeight));
                      }}
                      className="w-full accent-emerald-600 cursor-pointer h-2 bg-[#d7e4e2] rounded-lg"
                    />

                    {/* Standard Milestone Ticks */}
                    <div className="flex justify-between text-[10px] font-mono-numbers text-[#527078] px-0.5">
                      <span className={matrixProgressionDay === 1 ? 'font-bold text-emerald-800' : ''}>
                        D1 ({getProgressionTargetEff(1, simStyleNature, smvWeight)}%)
                      </span>
                      <span className={matrixProgressionDay === 6 ? 'font-bold text-emerald-800' : ''}>
                        D6 ({getProgressionTargetEff(6, simStyleNature, smvWeight)}%)
                      </span>
                      <span className={matrixProgressionDay === 15 ? 'font-bold text-emerald-800' : ''}>
                        D15 ({getProgressionTargetEff(15, simStyleNature, smvWeight)}%)
                      </span>
                      <span className={matrixProgressionDay === 25 ? 'font-bold text-emerald-800' : ''}>
                        D25 ({getProgressionTargetEff(25, simStyleNature, smvWeight)}%)
                      </span>
                      <span className={matrixProgressionDay === 40 ? 'font-bold text-emerald-800' : ''}>
                        D40 ({getProgressionTargetEff(40, simStyleNature, smvWeight)}%)
                      </span>
                    </div>

                    {/* Quick Milestones Button Strip */}
                    <div className="pt-1">
                      <span className="text-[10px] font-bold uppercase text-[#527078] block mb-1">
                        40-Day Matrix Quick Milestones:
                      </span>
                      <div className="grid grid-cols-5 sm:grid-cols-10 gap-1">
                        {[
                          ...
                          const eff = getProgressionTargetEff(m.d, simStyleNature, smvWeight);
                          const isSelected = matrixProgressionDay === m.d;
                          return (
                            <button
                              key={m.d}
                              type="button"
                              onClick={() => {
                                setMatrixProgressionDay(m.d);
                                if (m.d <= 6) setSimLcCurrentDay(m.d);
                                setTargetEffPct(eff);
                              }}
                              className={`p-1 rounded-lg text-center font-mono-numbers transition-all cursor-pointer border ${
                                isSelected
                                  ? 'bg-[#176f78] text-white font-black border-[#176f78] shadow-xs'
                                  : 'bg-white text-[#17343a] hover:bg-[#eaf3f2] border-[#cbdad7] font-semibold'
                              }`}
                              title={`Adopt Day ${m.d}: ${eff}% Target Efficiency`}
                            >
                              <div className="text-[10px] leading-tight font-bold">{m.label}</div>
                              <div className="text-[9px] opacity-90">{eff}%</div>
                            </button>
                          );
                        })}
                      </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-bold text-[#17343a]">Custom Target Efficiency:</span>
                      <span className="font-mono-numbers font-bold text-[#176f78]">
                        {targetEffPct}%
                      </span>
                    </div>
                    <input
                      ...
                      className="w-full accent-[#17343a] cursor-pointer h-2 bg-[#d7e4e2] rounded-lg"
                    />
                    <div className="flex items-center justify-between text-[10px]">
                      <span className="text-[#527078]">Custom Range: 20% – 100%</span>
                      <button
                        type="button"
                        onClick={() => {
                          setIsMatrixEffMode(true);
                          setTargetEffPct(getProgressionTargetEff(matrixProgressionDay, simStyleNature, smvWeight));
                        }}
                        className="text-emerald-700 font-bold hover:underline flex items-center gap-1 cursor-pointer"
                      >
                        <RotateCcw className="w-3 h-3" />
                        <span>Re-align with 40-Day Matrix (Day {matrixProgressionDay} → {getProgressionTargetEff(matrixProgressionDay, simStyleNature, smvWeight)}%)</span>
                      </button>
                    </div>
                  </div>
                )}

                {/* Standard Work Guidance Note */}
                <div className="text-[10px] text-[#4a6b70] bg-[#eef6f5] border border-[#cbe0dc] rounded-lg p-2 flex items-start gap-1.5">
                  <Info className="w-3.5 h-3.5 text-[#176f78] shrink-0 mt-0.5" />
                  <div>
                    <strong>Debonair Unit-02 IE Standard:</strong> Target efficiency follows the official Standard 40-Day Style Progression Matrix. Day 1 starts at {getProgressionTargetEff(1, simStyleNature, smvWeight)}%, ramps up to {getProgressionTargetEff(6, simStyleNature, smvWeight)}% by Day 6, and plateaus at {getProgressionTargetEff(40, simStyleNature, smvWeight)}% steady terminal output.
                  </div>
                </div>
              </div>
                      ...
                      <div className="flex items-center gap-1.5">
                        {[1, 2, 3, 4, 5, 6].map(dayNum => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setSimLcCurrentDay(dayNum);
                              setMatrixProgressionDay(dayNum);
                              if (isMatrixEffMode) {
                                setTargetEffPct(getProgressionTargetEff(dayNum, simStyleNature, smvWeight));
                              }
                            }}
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-4 rounded-2xl bg-white border border-[#e7e1d5] border-l-[4px] border-l-[#176f78] shadow-2xs">
              ...
              <span className="text-[10px] text-[#527078] font-medium mt-1 block">
                Steady Target: {targetEffPct}% ({isMatrixEffMode ? `Matrix Day ${matrixProgressionDay}` : 'Custom'})
              </span>
            </div>
