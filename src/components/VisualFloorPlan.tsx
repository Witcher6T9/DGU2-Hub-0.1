...
export function normalizeFloorName(floorStr?: string): string {
  ...
  if (lower.includes('padma')) return 'Padma Floor';
  if (lower.includes('meghna')) return 'Meghna Floor';
  if (lower.includes('karnophuli')) return 'Karnophuli Floor';
  if (lower.includes('korotoya')) return 'Korotoya Floor';
  if (lower.includes('shitalokshya')) return 'Shitalokshya Floor';
  if (lower.includes('turag')) return 'Turag Floor';
  return floorStr.trim();
}
...
export const VisualFloorPlan: React.FC<VisualFloorPlanProps> = ({
  ...
  // Extract all unique floors present in dataset ordered canonically:
  // Padma Floor : 06 Lines, Meghna Floor : 06 Lines, Karnophuli Floor : 05 Lines,
  // Korotoya Floor : 06 Lines, Shitalokshya Floor : 06 Lines, Turag Floor : 05 Lines
  const availableFloors = useMemo(() => {
    const floorsSet = new Set<string>();
    layoutPhysicalLines.forEach(l => {
      const fl = normalizeFloorName(l.floor);
      if (fl) floorsSet.add(fl);
    });
    const officialNames = CANONICAL_FLOORS_CONFIG.map(f => f.name);
    officialNames.forEach(f => floorsSet.add(f));
    const arr = Array.from(floorsSet);
    return arr.sort((a, b) => {
      const idxA = officialNames.indexOf(a);
      const idxB = officialNames.indexOf(b);
  ...
  const [selectedFloor, setSelectedFloor] = useState<string>(() => {
    if (initialFloor) {
      const normalized = normalizeFloorName(initialFloor);
      if (availableFloors.includes(normalized)) return normalized;
    }
  ...
  const floorLines = useMemo(() => {
    let result = layoutPhysicalLines.filter(l => normalizeFloorName(l.floor) === selectedFloor.trim());

    // If no lines found by floor string match, fallback to line number range from CANONICAL_FLOORS_CONFIG
    if (result.length === 0) {
      const cfg = CANONICAL_FLOORS_CONFIG.find(c => c.name === selectedFloor.trim());
      if (cfg) {
        result = layoutPhysicalLines.filter(l => {
          const num = parseInt(l.lineNo, 10);
          return cfg.lineNums.includes(num);
        });
      }
    }
